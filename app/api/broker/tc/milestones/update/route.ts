...
export async function PATCH(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    // Only agents and TCs can update milestones
    if (!['agent', 'tc'].includes(userRole)) {
      return NextResponse.json(
        { success: false, error: 'Unauthorized to update milestones' },
        { status: 403 }
      )
    }
    ...
    if (!hasAccess) {
      return NextResponse.json(
        { success: false, error: 'Unauthorized to update this milestone' },
        { status: 403 }
      )
    }
    ...
    console.error('Error in milestone update endpoint:', error)
    return NextResponse.json(
