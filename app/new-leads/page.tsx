...
export default function NewLeadsPage() {
  ...
  const getTypeColor = (type: string) => {
    switch (type?.toLowerCase()) {
      ...
      case 'investor': return { bg: 'bg-purple-500/10', text: 'text-purple-400', border: 'border-purple-500/20' }
      case 'renter': return { bg: 'bg-green-500/10', text: 'text-green-400', border: 'border-green-500/20' }
  ...
  const getUrgencyBadge = (urgency: string) => {
    switch (urgency) {
      case 'hot': return { bg: 'bg-red-500/15', text: 'text-red-400', label: 'HOT', icon: Flame }
      case 'warm': return { bg: 'bg-yellow-500/15', text: 'text-yellow-400', label: 'WARM', icon: Zap }
      ...
      <div className="max-w-3xl mx-auto px-6 py-6">
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-gray-500 text-sm mt-1">New leads will appear here when your broker posts them.</p>
          </div>
        ) : (
                ...
                >
                  ...
                  {lead.property_address && (
                    <div className="flex items-center gap-2 mt-3 pt-3 border-t border-white/5">
                      ...
                      <p className="text-sm text-gray-300">{lead.property_address}</p>
                    </div>
                  )}

                  {/* Notes */}
                  {lead.notes && (
                    <p className="text-sm text-gray-500 italic mt-2">{lead.notes}</p>
                  )}
                  ...
                  {mine && (
                    <div className="bg-[#2EC4D6]/5 border border-[#2EC4D6]/15 rounded-lg p-4 mt-3 space-y-2">
                      ...
                      {lead.phone && (
                        <a href={`tel:${lead.phone}`} className="flex items-center gap-2 text-sm text-gray-300 hover:text-[#2EC4D6] transition">
                          <Phone className="w-4 h-4 text-[#2EC4D6]" /> {lead.phone}
                        </a>
                      )}
                      {lead.email && (
                        <a href={`mailto:${lead.email}`} className="flex items-center gap-2 text-sm text-gray-300 hover:text-[#2EC4D6] transition">
                  ...
                  {/* Claimed by other */}
                  {claimed && !mine && (
                    <div className="flex items-center gap-2 mt-3 pt-3 border-t border-white/5 text-sm text-gray-500">
